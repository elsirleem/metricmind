...
2. Pydantic schemas
3. Ingestion service (GitLab + Jira connectors)
4. Metric computation service
5. Metric selection engine
6. Threshold checker
7. Snapshot assembler
8. LLM Call 2 — trade-off reasoning (most critical — validate first)
9. LLM Call 1 — profile interpretation
10. LLM Call 3 — explanation generation
11. FastAPI routers and endpoints
12. Next.js frontend pages
13. Mock data seed script
...
| Decision | Rationale |
|---|---|
| LLM provider switch | Single `LLM_PROVIDER` env variable controls which provider is used. No code changes needed to switch. Gemini used during development (free tier), Anthropic during evaluation (stronger JSON schema adherence). |
